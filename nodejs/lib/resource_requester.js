...
var EVENT_TRANSMISSION_ERROR = 'resource_transmission_error';
...
ResourceRequester.EVENT_TRANSMISSION_ERROR = EVENT_TRANSMISSION_ERROR;
...
var readResourceData = function(reqTask, data) {
	...
	if(typeof reqTask.received === 'undefined')
		reqTask.received = 0;
	reqTask.received += data.length;
	// TODO
	if (reqTask.received === reqTask.resourceSize) {
		...
		console.error("fetching resource: " + reqTask.resourceName
				+ " got error!");
		onResourceTransferError.call(this, reqTask);
...
var onResourceTransferError = function(task){
	var resource = {
			name : reqTask.resourceName,
			type : reqTask.resourceType,
			totalSize : reqTask.resourceSize
	};
	
	this.emit(EVENT_TRANSMISSION_ERROR, resource);
};
